...
export class Update extends plugin {
  ...
  async runUpdate (isForce) {
    let command = 'git -C ./plugins/nai-plugin/ pull --no-rebase'
    if (isForce) {
      command = `git -C ./plugins/nai-plugin/ checkout . && ${command}`
      this.e.reply('正在执行强制更新操作，请稍等')
    } else {
      this.e.reply('正在执行更新操作，请稍等')
    }
    /** 获取上次提交的commitId，用于获取日志时判断新增的更新日志 */
    this.oldCommitId = await this.getcommitId('nai-plugin')
    uping = true
    const ret = await this.execSync(command)
    uping = false

    if (ret.error) {
      logger.mark(`${this.e.logFnc} 更新失败：nai-plugin`)
      this.gitErr(ret.error, ret.stdout)
      return false
    }

    /** 获取插件提交的最新时间 */
    const time = await this.getTime('nai-plugin')

    if (/(Already up[ -]to[ -]date|已经是最新的)/.test(ret.stdout)) {
      await this.reply(`nai-plugin已经是最新版本\n最后更新时间：${time}`)
    } else {
      await this.reply(`nai-plugin\n最后更新时间：${time}`)
      this.isUp = true
      /** 获取nai-plugin的更新日志 */
      const log = await this.getLog('nai-plugin')
